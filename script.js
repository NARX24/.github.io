...
window.keisan = function() { // グローバルスコープに公開
    ...
    items.forEach(item => {
        ...
        if (checkbox) {
            ...
            const isContainedInFinalSet = !isFinalSelection && [...finalSelectedSets].some(setId => getFinalParts(setId).includes(item.id));
            ...
            if (shouldBeDisabled || isContainedInFinalSet) {
